...
public class ChessBoardView extends View {
	...
	public void movePiece(int fromOffset, int toOffset) {
		...
		this.invalidate(getPixels(SIZE_TILE * tx), getPixels(SIZE_TILE * ty), getPixels(SIZE_TILE * (tx + 1)), getPixels(SIZE_TILE * (ty + 1)));
	}
	...
	public void onDraw(Canvas canvas) {
		...
		for (int x = 0; x < 8; x++) {
			for (int y = 0; y < 8; y++) {
				rectDestination.set(pxSize * x, pxSize * y, pxSize * (x + 1), pxSize * (y + 1));
				tile = getTilePiece(x, y);
				if (isSelected(x, y) || isSelectionTarget(x, y)) {
					canvas.drawRect(rectDestination, (isSelectionTarget(x, y) ? paintHighlightTargetContext : paintHighlightContext));
				}
				if (tile != 0) {
					pieceTransMatrix.setRectToRect(rectSource, rectDestination, Matrix.ScaleToFit.END);
	...
	public void init() {
		...
		Bitmap sampleBitmap;
		bitmaps[ID_BOARD] = BitmapFactory.decodeResource(r, R.drawable.tempchessboard);
		bitmaps[ID_BPAWN] = BitmapFactory.decodeResource(r, R.drawable.pawn_black);
		bitmaps[ID_BROOK] = BitmapFactory.decodeResource(r, R.drawable.castle_black);
		bitmaps[ID_BKNIGHT] = BitmapFactory.decodeResource(r, R.drawable.knight_black);
		bitmaps[ID_BBISHOP] = BitmapFactory.decodeResource(r, R.drawable.bishop_black);
		bitmaps[ID_BQUEEN] = BitmapFactory.decodeResource(r, R.drawable.queen_black);
		bitmaps[ID_BKING] = BitmapFactory.decodeResource(r, R.drawable.king_black);
		bitmaps[ID_WPAWN] = BitmapFactory.decodeResource(r, R.drawable.pawn_white);
		bitmaps[ID_WROOK] = BitmapFactory.decodeResource(r, R.drawable.castle_white);
		bitmaps[ID_WKNIGHT] = BitmapFactory.decodeResource(r, R.drawable.knight_white);
		bitmaps[ID_WBISHOP] = BitmapFactory.decodeResource(r, R.drawable.bishop_white);
		bitmaps[ID_WQUEEN] = BitmapFactory.decodeResource(r, R.drawable.queen_white);
		bitmaps[ID_WKING] = BitmapFactory.decodeResource(r, R.drawable.king_white);
		...
		// Create the required objects for the bitmap draw transformation matrix.
		// We need to scale and translate the bitmap. The setScale() and setTranslate()
		// methods overwrite the matrix state, i.e. you cannot use both together.
		// Thus, we need to use the Matrix.setRectToRect, which combines scale and translate.
		...
		rectSource = new RectF(0F, 0F, sampleBitmap.getWidth(), sampleBitmap.getHeight());
		
		// Second argument is the rectangle to fit into the destination.
		// This will change depending on where on the board the piece is being placed.
		// We will initialize it with default values, but they will need to be overwritten
		// when painting is performed. Note that we create this here because we don't want
		// to create new rectangle objects on every repaint.
		rectDestination = new RectF(rectSource);
		
		// The transformation matrix itself needs to be defined for the same reason that we
